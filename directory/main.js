...
const renderCarousel = async () => {
  ...
  const isImageExists = await checkImage(
    productsList[highestWeekBuysIdx].imageUrl
  );
  ...
  const highestWeekViews = productsList.reduce(
    (max, product) =>
      product.scores.week.views > max ? product.scores.week.views : max,
    productsList[0].scores.week.views
  );
